// Enforces the @underlying/timeline size budget on the built ESM bundle (gzip,
...
//
// The timeline is pure glue over the public playback surface (no new physics,
// no new scheduler): a position parser, lazy build + bake orchestration, and a
// master PlaybackHandle shell. The whole surface is the gate.
...
const full = await probe(`import * as timeline from './index.js'\nconsole.log(timeline)`)
check('@underlying/timeline (full surface)', full, FULL_BUDGET_BYTES)
